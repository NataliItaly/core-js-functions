...
function getPolynom(...coefficients) {
  ...
  return (x) => {
    const reversedCoef = [...coefficients].reverse();
    let res = 0;
    for (let i = 0; i < reversedCoef.length; i += 1) {
      res += reversedCoef[i] * x ** i;
    }
    return res;
  };
}
...
function retry(func, attempts) {
  return function throwErrors(...args) {
    let lastError;
